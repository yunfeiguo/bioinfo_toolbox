...
public class KruskalMST implements MST{
  ...
  private UF uf; //union-find data structure, used for checking cycle
  ...
  public KruskalMST (EdgeWeightedGraph g) {
    ...
    totalWeight = 0;
    java.util.Queue<Edge> q = new PriorityQueue<>(g.V());
    for (Edge e : g.edges()) {
      q.offer(e);
    }
    while(!q.isEmpty()) {
      ...
      int t = next.other(s);
      if (uf.connected(s, t))
        continue; //adding this edge will result in a cycle
      uf.union(s, t);
      mst.add(next);
