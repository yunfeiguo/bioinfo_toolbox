...
public class IndexMinPQ<Key extends Comparable<Key>> implements Iterable<Integer>{
    ...
    private int[] pq;        // binary heap using 1-based indexing, its value is index to keys
    ...
    public IndexMinPQ(int maxN) {
        ...
        this.maxN = maxN;
        keys = (Key[]) new Comparable[maxN + 1]; //who do we add 1 here?
        pq = new int[maxN + 1];
        qp = new int[maxN + 1];
    ...
    public void delete(int i) {
        if (i < 0 || i >= maxN) throw new IndexOutOfBoundsException();
        if (!contains(i)) throw new NoSuchElementException("index is not the in the priority queue");
        int index = qp[i];
        N--;
        exch(index, N + 1); //swap index on pq, which is 1-based        
        sink(index);
        swim(index);
     ...
     ***************************************************************************/
      ...
      ***************************************************************************/
     /*
      * switch with parents until no more larger parents are found
      */
     private void swim(int i) {
         //pq is 1-based heap indexing
         //i is index of pq
         while(i >= 2 && greater(i/2, i)) {
             exch(i/2, i);
             i = i/2;
         }
     }
     /*
      * swith with smaller children until no more smaller children found
      */
     private void sink(int i) {         
         while (true) {
             int child = i*2;
             //choose the smaller children
             if (child + 1 <= N && greater(child, child + 1)) {
                 child += 1;
             }
             //swap with children if there are children and 
             //parents > children
             //otherwise stop here
             if (child <= N && greater(i, child)) {
                 exch(i, child);
                 i = child;
             } else {
                 break;
