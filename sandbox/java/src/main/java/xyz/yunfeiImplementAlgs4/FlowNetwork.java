...
public class FlowNetwork {
  ...
  private int V;
  private int E;
  private List<List<FlowEdge>> neighbors;
  ...
  public FlowNetwork(int V) {
    if (V < 0) throw new IllegalArgumentException("negative # of nodes");
    this.V = V;
    E = 0;
  ...
  public FlowNetwork(int V, int E) {
    this(V);
    if (E < 0) throw new IllegalArgumentException("negative # of edges");
    for (int i = 0; i < E; i++) {
  ...
  public void addEdge(FlowEdge e) {
    ...
    validateVertex(from);
    validateVertex(to);
    neighbors.get(from).add(e);
    //why add edges to both ends? because we will also deal with residual edge
    neighbors.get(to).add(e);
    E++;
  }
  ...
  public Iterable<FlowEdge> edges() {
    List<FlowEdge> allEdges = new ArrayList<>(E);
    for (int i = 0; i < V; i++) {
      for (FlowEdge e : adj(i)) {
        //only add i->other edges, not the reverse
        if (e.from() == i) {
          allEdges.add(e);
        }
      }
  ...
  public String toString() {
    ...
    for (int v = 0; v < V; v++) {
      s.append(v + ":  ");
      for (FlowEdge e : neighbors.get(v)) {
