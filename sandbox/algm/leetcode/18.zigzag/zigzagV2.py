    ...
    def convertIdx(self,i,n,l):
        ...
        if mod >= n: #not in vertical lines
            ...
            if mod == 0:
                ...
                #it is necessary to adjust for last col and last zag part (the slope)
                finalMod = (l-1) % (2*n-2)
                adjust = self.getAdjust(finalMod,mod,n)
        ...
        r = int(r)
        return(r)
    def getAdjust(self,finalMod,mod,n):  
        adjust = 0
        if finalMod >= mod - 1: #final vertical line goes beyond row mod-1
            adjust += mod
        else:
            adjust += finalMod + 1
        if finalMod > 2*(n-1) - mod:
            #adjust for elements on zag part (last zag beyond last column)
            adjust += finalMod - (2*(n-1)-mod)
        return(adjust)
