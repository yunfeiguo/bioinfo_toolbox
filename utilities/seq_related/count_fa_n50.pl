...
foreach my $file (@data_files){
	...
	{
	    $seqlen{$Id} += length($_) if $Id; 
	    $seqgc{$Id} += ($_ =~ tr/gGcC/gGcC/) if $Id;
	}
    }
    close (FASTA);
}
...
my %len= ();
my @seqLengths;
foreach my $id (keys %seqlen) {
    push @seqLengths, $seqlen{$id}; # record length for N50 calc's
    $n++;
    $int = floor( $seqlen{$id}/$opt_i );
    $totalLength += $seqlen{$id};
    $gcCount += $seqgc{$id};
    if( !defined($len{$int}) ) {
	$len{$int} = 1;
    } else {
